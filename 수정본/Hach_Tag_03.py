...
class WindowClass(QMainWindow, form_class) :
    ...
    def __init__(self) :
        ...
        #+버튼
        self.plus.clicked.connect(self.Edit)
    ...
    def Edit(self):
        W_text = self.OutText.toPlainText() #텍스트박스 문자열
        W_Line = W_text.split('#') #텍스트 문자열 리스트화
        print(W_Line)

        #샵 다시 추가
        for i in W_Line:
            if W_Line[0] == i: #첫번째 건너 뛰는 용도
                continue
            W_Line[W_Line.index(i)] = '#' + i #2번쨰 단어들은 해쉬태그 다시 붙임
            #이 과정 거치면 ["문장" "#단어", "#단어"]
        ...
        if len(W_Line) > 2:
            for i in W_Line:
                if W_Line[0] == i or W_Line[1] == i:
                    continue
                W_Line[1] = W_Line[1] + i  #해시태그 달린거 끼리 합침
